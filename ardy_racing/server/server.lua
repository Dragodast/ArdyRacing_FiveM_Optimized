local RunningEvents = {}
local EventIdCounter = 1000
local EventIdLock = false

math.randomseed(os.time())
--[[ math.random(); math.random(); math.random() ]]
local AdminKey = math.random(10000, 99999)

function GetNextEventIdSafe()
    while EventIdLock  do
        Wait(0)
    end
...
function ProcessLeaderboard(event, playerData, sourceCopy, startVehicle, finisVehicle)
    CreateThread(function() 
        ...
        if evt.IsRanked  then
            return
        end

        if (not startVehicle or not startVehicle.Hash) then
            print('Aracing - ProcessLeaderboard - missing start vehicle data')
            return
        end

        if (not finisVehicle or not finisVehicle.Hash) then
        ...
        if (evt.IsVerified) then
            ...
            if result.Car > 0 and Config.enable_car_specific_leaderboard_notif  then
...
AddEventHandler("ardy_racing:AdminLogin", function(pass)
    local src = source
    if pass ~= Config.AdminPassword then
...
function SortEventPlayers(event)
    ...
    table.sort(event.Players, function(left, right) 
        if left.DNF  and right.DNF  then
            return true
        elseif left.DNF  and right.DNF  then
            return false
        end

        if left.HasFinished  and right.HasFinished  then
            return false
        elseif left.HasFinished  and right.HasFinished  then
            return true
        end
            ...
            for eventIndex, event in pairs(RunningEvents) do
                ...
                elseif cleanRaceTime > 0 then
                    for _, p in pairs(event.Players) do
                        if p.DNF  and p.HasFinished  and currentTime - p.LastBeat > 10000 then
                            p.DNF = true
                            p.HadFinished = true
                    ...
                    for index, p in pairs(event.Players) do
                        ...
                        if p.HasFinished  and p.DNF  then
                            isAnyoneRacing = true
                        end
                    end
                    ...
                    if event.WinnerAnnounced  and firstPlayer.HasFinished  and (event.Type == RACETYPE_SPRINT or event.Type == RACETYPE_CIRCUIT) then
