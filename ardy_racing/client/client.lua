...
local raceList = nil
local raceListTitle = nil

local availableEvents = {}
local isCountdownFrozen = false

local notificationsBlocked = GetResourceKvpInt('cnf_notificationsBlocked') == 1
local debugMode = GetResourceKvpInt('cnf_debugMode') == 1
local alwaysAllBlips = GetResourceKvpInt('cnf_alwaysAllBlips') == 1

local menuDefaultSprite =
{
...
function RegenerateMenu(forceReopen)
    ...
    if wasOpen  or forceReopen  then
        OpenMenu()
    end
end
...
AddEventHandler("ardy_racing:NewRaceEvent", function(raceEvent)
    ...
    if raceEvent.EventCreatorServerId ~= GetPlayerServerId(PlayerId()) and notificationsBlocked  and raceEvent.IsPublic  then
...
AddEventHandler("ardy_racing:AnnounceRaceWinner", function(eventUID, eventType, player)
    ...
    elseif eventType == RACETYPE_DRIFTCIRCUIT then
        if not player.BestLapDrift then
...
AddEventHandler("ardy_racing:PlayerDisqualified", function(eventUID, playerServerId, reason)
    if not currentRace or currentRace.EventUID ~= eventUID then
        return
    end
...
AddEventHandler("ardy_racing:NewRecordSet", function(playerServerId, isAllCars, position, eventName, eventType, carHash, record, playerName)
    ...
    if playerServerId ~= GetPlayerServerId(PlayerId()) and notificationsBlocked  then
        return
    end
...
AddEventHandler("ardy_racing:OpenRaceLeaderboards", function(raceName, raceType, leaderboard)
    ...
    local isHigherBetter = leaderboard['IsHigherBetter'] 
    ...
    if leaderboard[ALLCAR] ~= nil then
        table.sort(leaderboard[ALLCAR], function(left, right) return (isHigherBetter  and left.Record > right.Record) or (isHigherBetter == false and left.Record < right.Record) end)
    ...
    for key, cat in pairs(leaderboard) do
        ...
        if key ~= ALLCAR and key~= 'IsHigherBetter' and not carHash then
            local carName = GetDisplayNameFromVehicleModel(carHash)
            ...
            table.sort(cat, function(left, right) return (isHigherBetter  and left.Record > right.Record) or (isHigherBetter == false and left.Record < right.Record) end)
                ...
                if IsPedInAnyVehicle(player, false) then
                    ...
                    if raceCleanTime < 0 then
                        ...
                        if isCountdownFrozen == false then
                            ...
                            if currentRace.AllowSlipstream  then
                    ...
                    elseif raceCleanTime >= 0 and isCountdownFrozen  then
                    ...
                    if checkpointProximity < (Config.checkpoint_radius / 2) * 1.2 then                        
                        ...
                        if (currentRace.CheckpointCurrent == #currentRace.Checkpoints and not currentRace.CurrentLap) or (currentRace.CheckpointCurrent == 1 and not currentRace.CurrentLap and currentRace.CurrentLap == currentRace.Laps) then
                    ...
                    if currentRace.AllowExitCar  then
        ...
        for index, event in pairs(availableEvents) do
            ...
            if event.StartTime > currentTime then
                ...
                if proximity < 80 then
                    ...
                    if IsControlJustReleased(1, 38) and ((exports.ardy_easymenu:IsAnyMenuVisible() == false and currentState <= STATE_PREJOIN) or (exports.ardy_easymenu:IsAnyMenuVisible()  and currentState < STATE_PREJOIN)) then
...
function OpenMenu()
    if currentState == STATE_INSPECT then
        if exports.ardy_easymenu:ToggleMenuVisible(GetCurrentResourceName())  then
            return
        else
            currentState = STATE_NONE
            SetWaypointOff()
                    ...
                    Buttons = {
                        {Name = 'Car drift tires', NameRight = GetBoolTextDriftTyres(driftTyre), FuncOnSelected = function(buttonRef) 
                            if gameBuild >= 2189 then
                                if Config.enable_drift_tire_manipultion  then 
                                    local player = PlayerPedId()
        ...
        menu = 
        {
            ...
            Buttons = 
            {
                ...
                {Name = 'Listed publicly', NameRight = GetBoolYesNoTextInverted(currentRace.IsUnlisted), FuncOnSelected = function(buttonRef)
                    if not currentRace.IsUnlisted then
    ...
    elseif currentState == STATE_PREJOIN then
        ...
        if Config.enable_teleport_to_race_start  then
...
function OpenListMenu()
    ...
    if not raceList and #raceList > 0 then
        ...
        for _, race in pairs(raceList) do
            ...
            if not raceTypeSubmenus[raceTypeText] then
    ...
    local menu = 
            ...
            Buttons = 
            {
                ...
                {Name = 'Listed publicly [Editable]', NameRight = GetBoolYesNoTextInverted(race.IsUnlisted), FuncOnSelected = function(buttonRef)
                    if currentRace ~= nil then
                        ...
                        TriggerServerEvent("ardy_racing:SetRaceUnlisted", not currentRace.IsUnlisted , currentRace)
        ...
        if isAdmin  then
            table.insert(menu.Buttons, #menu.Buttons, {Name = '[Admin] Verified', NameRight = GetBoolYesNoText(race.IsVerified), FuncOnSelected = function(buttonRef)
                if currentRace ~= nil then
                    TriggerServerEvent("ardy_racing:VerifyRace", not currentRace.IsVerified , currentRace, adminKey)
...
function GenerateCurrentRacePlayers(menu)
    if not currentRace or not currentRace.Players or #currentRace.Players <= 0 then
        return
    end
...
function GetBoolYesNoTextInverted(boolVar)
    if boolVar  then -- Nil returns yes, this saves mi some logic outside
        return 'YES'
    else
        return 'NO'
    end
...
function RaceLapFinished(currentTime)
    ...
    if not currentRace or not currentRace.CurrentLap then
        return
    end
...
function GetRaceHudData()
    if not currentRace or not currentRace.StartTime then
        return nil
    end
    local checkpointNum = currentRace.CheckpointCurrent - 1
    if checkpointNum <= 0 then
    ...
    local myPos = 1
    if not myPlayer and not myPlayer.Position then
    ...
    if IsRaceDriftType()  then
        ...
        if currentRace.Type == RACETYPE_DRIFTCIRCUIT then
            if not currentRace.BestDriftLap then
                driftScoreBestLap = tostring(currentRace.BestDriftLap) .. ' pts'
            else
                driftScoreBestLap = '---'
            end

            if not currentRace.LastDriftLap then
