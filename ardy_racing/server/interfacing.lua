...
function GetLeaderboardPosition(raceName, raceType, record)
    ...
    table.sort(LeaderboardData.Races[raceName][ALLCAR], function(left, right) return (isHigherBetter  and left.Record > right.Record) or (isHigherBetter == false and left.Record < right.Record) end)
    table.sort(LeaderboardData.Races[raceName][carHashString], function(left, right) return (isHigherBetter  and left.Record > right.Record) or (isHigherBetter == false and left.Record < right.Record) end)
    ...
    for index, k in pairs(LeaderboardData.Races[raceName][ALLCAR]) do
        if k.AuthorName == record.AuthorName and k.CarHash == record.CarHash then
            overallFound = true
            if (isHigherBetter  and record.Record > k.Record) or (isHigherBetter == false and record.Record < k.Record) then
...
function RenameRace(oldName, newName)
    while LeaderboardLocked  or DataLocked  do
        Wait(0)
    end
